...
class MetadataApi {
  ...
  ///   The ID of the exchange.
  ///
  /// * [String] exchangeSymbolId (required):
  ///   The exchange-native symbol identifier (`symbol_id_exchange`).
  Future<Response> v1SymbolsExchangeIdByExchangeSymbolExchangeSymbolIdGetWithHttpInfo(String exchangeId, String exchangeSymbolId, { Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/symbols/{exchange_id}/by-exchange-symbol/{exchange_symbol_id}'
      .replaceAll('{exchange_id}', exchangeId)
      .replaceAll('{exchange_symbol_id}', exchangeSymbolId);
  ...
  /// * [String] exchangeSymbolId (required):
  ///   The exchange-native symbol identifier (`symbol_id_exchange`).
  Future<MarketDataMetadataSymbol?> v1SymbolsExchangeIdByExchangeSymbolExchangeSymbolIdGet(String exchangeId, String exchangeSymbolId, { Future<void>? abortTrigger, }) async {
    final response = await v1SymbolsExchangeIdByExchangeSymbolExchangeSymbolIdGetWithHttpInfo(exchangeId, exchangeSymbolId, abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
    ...
    // FormatException when trying to decode an empty string.
    if (response.body.isNotEmpty && response.statusCode != HttpStatus.noContent) {
      return await apiClient.deserializeAsync(await _decodeBodyBytes(response), 'MarketDataMetadataSymbol',) as MarketDataMetadataSymbol;
  ...
  /// * [int] limit:
  ///   Number of records to return per page.
  Future<Response> v1SymbolsExchangeIdUnmappedGetWithHttpInfo(String exchangeId, { int? page, int? limit, Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
    final path = r'/v1/symbols/{exchange_id}/unmapped'
      .replaceAll('{exchange_id}', exchangeId);
  ...
  /// * [int] limit:
  ///   Number of records to return per page.
  Future<List<MarketDataMetadataSymbol>?> v1SymbolsExchangeIdUnmappedGet(String exchangeId, { int? page, int? limit, Future<void>? abortTrigger, }) async {
    final response = await v1SymbolsExchangeIdUnmappedGetWithHttpInfo(exchangeId, page: page, limit: limit, abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
